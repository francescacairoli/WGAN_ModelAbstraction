...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.conv_blocks = nn.Sequential(
            nn.ConvTranspose1d(self.Nch+opt.y_dim, 128, 4, stride=2, padding=self.padd),
    ...
    def forward(self, noise, conditions):
        ...
        conds_rep = conds_flat.repeat(1,self.Q).view(conditions.shape[0], opt.y_dim, self.Q)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            *discriminator_block(opt.x_dim, 64, (opt.traj_len+opt.y_dim)//2),
            *discriminator_block(64, 64, ((opt.traj_len+opt.y_dim)//2)//2)
        )

        # The height and width of downsampled image
        ds_size = (opt.traj_len + opt.y_dim) // (2**2)
...
plots_path = model_name+"/MA_w_Plots/ID_"+ID
